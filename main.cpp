...
int main(int argc, char *argv[]) {
	...
	mosqpp::lib_init();
	mqtt_client *iot_client = new mqtt_client("CLIENT", "localhost", 1883, 60);
	while (s_running) {
		l_rc = iot_client->loop();
		if (l_rc) {
			l_subcribed = false;
		...
		if (!l_subcribed) {
			for (const auto& l_topic : l_topics) {
				...
				iot_client->subscribe(NULL, l_topic.c_str());
				iot_client->send_message(l_topic, "LISTENING");
			}
			l_subcribed = true;
		} else {
			for (const auto& l_topic : l_topics) {
				iot_client->send_message(l_topic);
